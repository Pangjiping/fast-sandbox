...
func TestServerRelaysToSignedTargetPort(t *testing.T) {
	...
	require.NoError(t, err)
	preamble, err := dataplane.EncodeLocalForwardPreamble(uint32(backendPort), credential)
	require.NoError(t, err)
	require.NoError(t, dataplane.WriteLocalForwardPreamble(connection, preamble))
...
func TestServerRejectsReservedTunnelPort(t *testing.T) {
	...
	connection, err := net.DialTimeout("tcp", tunnel.Addr().String(), time.Second)
	require.NoError(t, err)
	preamble, err := dataplane.EncodeLocalForwardPreamble(uint32(port), credential)
	require.NoError(t, err)
	require.NoError(t, dataplane.WriteLocalForwardPreamble(connection, preamble))
...
func TestServerHealthHandshakeDoesNotDialGuestPort(t *testing.T) {
	...
	connection, err := net.DialTimeout("tcp", tunnel.Addr().String(), time.Second)
	require.NoError(t, err)
	preamble, err := dataplane.EncodeLocalForwardHealthPreamble(credential)
	require.NoError(t, err)
	require.NoError(t, dataplane.WriteLocalForwardPreamble(connection, preamble))
	require.NoError(t, connection.SetReadDeadline(time.Now().Add(time.Second)))
	buffer := make([]byte, 1)
...
func TestServerRejectsCredentialFromAnotherBox(t *testing.T) {
	...
	connection, err := net.DialTimeout("tcp", tunnel.Addr().String(), time.Second)
	require.NoError(t, err)
	preamble, err := dataplane.EncodeLocalForwardPreamble(8080, otherCredential)
	require.NoError(t, err)
	require.NoError(t, dataplane.WriteLocalForwardPreamble(connection, preamble))
	require.NoError(t, connection.SetReadDeadline(time.Now().Add(time.Second)))
	buffer := make([]byte, 1)
