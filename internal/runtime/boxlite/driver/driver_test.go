...
func TestDriverSidecarContract(t *testing.T) {
	...
	testAccess := dataplane.AccessDescriptor{
		Kind: dataplane.AccessKindLocalForward, Address: "127.0.0.1:21000", Credential: credential,
	}
	var mu sync.Mutex
...
func TestDriverCapabilityAndIdentityFailClosed(t *testing.T) {
	handler := http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "/v1/boxes/uid-conflict":
			...
			writeBoxLiteTestJSON(t, writer, boxLiteErrorResponse{Code: boxliteprotocol.ErrorImmutableSpecConflict, Message: "resource hash changed"})
		default:
			http.NotFound(writer, request)
		}
	})
...
func newBoxLiteTestDriver(t *testing.T, handler http.Handler) *Driver {
	t.Helper()
