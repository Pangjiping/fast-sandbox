...
func EncodeLocalForwardPreamble(targetPort uint32, credential string) ([]byte, error) {
	return dataplane.EncodeLocalForwardPreamble(targetPort, credential)
}

func newLocalForwardTransport(access dataplane.AccessDescriptor, targetPort uint32, dial DialContextFunc) (*http.Transport, error) {
	...
	return &http.Transport{
		...
		DialContext: func(ctx context.Context, _, _ string) (net.Conn, error) {
			...
			if err := dataplane.WriteLocalForwardPreamble(connection, preamble); err != nil {
				_ = connection.Close()
