...
func TestLocalForwardTransportRejectsNonLoopbackEndpoint(t *testing.T) {
	credential, err := dataplane.GenerateLocalForwardCredential()
	require.NoError(t, err)
	_, err = newLocalForwardTransport(dataplane.AccessDescriptor{
		Kind: dataplane.AccessKindLocalForward, Address: "10.0.0.8:19090", Credential: credential,
	}, 8080, nil)
...
func TestProxyForwardsThroughLocalTunnelWithSignedTargetPort(t *testing.T) {
	...
	go func() {
		...
		decodedPort, readErr := dataplane.DecodeLocalForwardPreamble(connection, credential)
		...
		if request.URL.Path != "/health" || request.Header.Get("Authorization") != "" || request.Header.Get(dataplane.HeaderRouteGeneration) != "" {
